...
**Purpose**: Automatically detect the cleave column and split the buffer — the simplest entry point for two-column files.

**Preconditions**: Original buffer must be saved (no unsaved changes).

**Column detection priority**:
1. **Modeline**: If the buffer has a cleave modeline with `cc=N`, use that column. (Same as existing modeline path in `CleaveAtCursor`.)
2. **Gap detection**: Scan buffer lines to find the column where left content ends and right content begins.
3. **New file fallback**: If gap detection finds no two-column layout, cleave after the longest line plus the gutter margin, creating an empty right buffer for new margin notes.

#### Gap detection algorithm

1. For each line, find all internal whitespace runs of length ≥ `g:cleave_gutter` (i.e., runs of spaces that have non-whitespace characters on both sides). Record the starting virtual column of each run.
2. Discard lines that have no qualifying gap (single-column lines, blank lines, lines where all content is contiguous).
3. Among lines with a qualifying gap, cluster the gap-start columns. The cleave column is the most common gap-start column (mode). On ties, prefer the leftmost.
4. If fewer than 2 lines share the winning column (±1 tolerance), fall through to the new file fallback.

**Tolerance**: Gap-start columns within ±1 of each other are treated as the same cluster. This accommodates ragged left-side line endings where trailing content may shift the gap by one column.

#### New file fallback (no existing right content)

When gap detection does not find a two-column layout, the file is treated as single-column content that needs a fresh right margin:

1. Compute the display width of the longest line in the buffer (ignoring trailing whitespace).
2. Set the cleave column to: `longest_line_width + g:cleave_gutter + 1`.
3. If `&textwidth > 0` and `&textwidth < longest_line_width`, use `&textwidth` instead of `longest_line_width` (respects the user's wrap width preference).

This produces a left buffer containing all existing content and an empty right buffer ready for margin notes.

**Behavior after detection**: Delegates to the same `SplitBufferAtCol` path used by `CleaveAtCursor` / `CleaveAtColumn`.

**Command**: `:Cleave`

**Effects**: Same as `CleaveAtCursor` once the column is determined.
