...
usage() {
    cat <<'EOF'
Usage: test/test_paragraph_ops_fuzz.sh [options]
...
echo "Running Cleave paragraph ops fuzz tests"
echo "  seed: $SEED"
...
VIM_CMD+=(
    -c "source test/fuzz/paragraph_ops_fuzz.vim"
    -c "if RunParagraphOpsFuzz() | cquit 1 | endif"
    -c "qa!"
)

if "${VIM_CMD[@]}"; then
    echo "Paragraph ops fuzz tests passed."
    exit 0
fi

echo "Paragraph ops fuzz tests FAILED."
if [ -z "$REPLAY_FILE" ]; then
    echo "Re-run with:"
    echo "  test/test_paragraph_ops_fuzz.sh --seed $SEED"
    echo "    --iterations $ITERATIONS"
fi
