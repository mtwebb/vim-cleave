...
usage() {
    cat <<'EOF'
Usage: test/test_reflow_fuzz.sh [options]
...
echo "Running Cleave reflow fuzz tests"
echo "  seed: $SEED"
...
if "${VIM_CMD[@]}"; then
    echo "Reflow fuzz tests passed."
    exit 0
fi

echo "Reflow fuzz tests FAILED."
if [ -z "$REPLAY_FILE" ]; then
    echo "Re-run with: test/test_reflow_fuzz.sh --seed $SEED --iterations $ITERATIONS"
fi
