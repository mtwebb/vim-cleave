...
usage() {
    cat <<'EOF'
Usage: test/test_split_join_fuzz.sh [options]
...
echo "Running Cleave split/join fuzz tests"
echo "  seed: $SEED"
...
VIM_CMD+=(
    -c "source test/fuzz/split_join_fuzz.vim"
    -c "if RunSplitJoinFuzz() | cquit 1 | endif"
    -c "qa!"
)

if "${VIM_CMD[@]}"; then
    echo "Split/join fuzz tests passed."
    exit 0
fi

echo "Split/join fuzz tests FAILED."
if [ -z "$REPLAY_FILE" ]; then
    echo "Re-run with:"
    echo "  test/test_split_join_fuzz.sh --seed $SEED --iterations $ITERATIONS"
fi
